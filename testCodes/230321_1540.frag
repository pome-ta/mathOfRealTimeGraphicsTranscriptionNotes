...
float converter(float v) { // 階調変換関数
  float twoTone; // 中央下: (a) 二階調化
  float posterization; // 右下  : (b) ポスタリゼーション
  float toneCurve; // 左上  : (c) S 字トーンカーブ
  float gammaCorrection; // 中央上: (d) ガンマ補正
  float solarisation; // 右上  : (e) ソラリゼーション
  float master; // 左下  :     元画像
  ...
  twoTone = step(time, v);
  posterization = (floor(n * v) + step(0.5, fract(n * v))) / n;
  toneCurve = smoothstep(0.5 * (1.0 - time), 0.5 * (1.0 + time), v);
  gammaCorrection = pow(v, 2.0 * time);
  solarisation = 0.5 * sin(4.0 * PI * v + u_time) + 0.5;
  master = v;
  
  return channel == ivec2(1, 0) ? twoTone : channel == ivec2(2, 0) ? posterization : channel == ivec2(0, 1) ? toneCurve : channel == ivec2(1, 1) ? gammaCorrection : channel == ivec2(2, 1) ? solarisation : master;
}
