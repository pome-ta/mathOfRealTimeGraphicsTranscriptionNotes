...
vec3 blend(float a, float b) {
  ...
  vec3[2]col2 = vec3[](
    vec3(a, a, 1), // a の値を青と白の中間色に変換
    vec3(0, b, b)// b の値を黒と緑の中間色に変換
  );
  return channel == 0 ? mix(col2[0], col2[1], time) : // 左: 一様な補間
  mix(col2[0], col2[1], smoothstep(0.5 - 0.5 * time, 0.5 + 0.5 * time, b / (a + b))); // 右: a と b の値に応じた補間
}
